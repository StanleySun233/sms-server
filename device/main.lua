...
util_webhook = require "util_webhook"
...
local last_incoming_phone, last_incoming_time
...
sys.subscribe("CC_IND", function(status)
    ...
    if status == "INCOMINGCALL" then
        ...
        is_calling = true
        last_incoming_phone = cc.lastNum()
        last_incoming_time = os.date("%Y-%m-%dT%H:%M:%S")
    ...
    if status == "DISCONNECTED" then
        if last_incoming_phone then
            util_webhook.addMissedCall(last_incoming_phone, last_incoming_time)
            last_incoming_phone, last_incoming_time = nil, nil
        end
        is_calling = false
